...
Flujo: router → retrieve → static → reviewer → synthesize → END.
Los nodos static/reviewer se auto-omiten según la ruta.
"""
...
def get_graph(settings: Settings | None = None):
    ...
    g.add_edge("retrieve", "static")
    g.add_edge("static", "reviewer")
    g.add_edge("reviewer", "synthesize")
